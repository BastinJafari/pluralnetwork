...
const useMutate = () => {
  ...
  const voteMutationNew = useMutation({
    mutationFn: ({ vote, id, increment }: any) => postVote({dir: vote, id: id, isPremium: premium?.isPremium}),
      ...
      if (data.id.substring(0, 3) === "t3_") {
        if (session?.user?.name) {
          ...
          queryClient.invalidateQueries({
            queryKey: ["feed", "SELF", session.user.name, "upvoted",]
          })
          data.vote == -1 &&
          queryClient.invalidateQueries({
            queryKey: [
  ...
  const saveMutation = useMutation(
      {
        mutationFn:     ({ id, isSaved }: any) => saveLink({id, isSaved, category: "", isPremium: premium?.isPremium}),
        onMutate: async (update) => {
          if (update.id.substring(0, 3) === "t3_") {
            return optimisticUpdate(["feed"], update.id, {
              property: "saved",
  ...
  const hideMutation = useMutation(
      {
        mutationFn:({ id, isHidden }: any) => hideLink({id, isHidden, isPremium: premium?.isPremium}),
  ...
  const postCommentMutation = useMutation({
    mutationFn: ({ parent, textValue, postName }: any) => postComment({parent, text: textValue, isPremium: premium?.isPremium}),
    onSuccess: (data: any) => {
      if (session?.user?.name) {
      ...
      if (data?.parent_id?.substring?.(0, 3) === "t3_") {
        ...
        queryClient.setQueriesData(
            {queryKey: ["thread", data?.parent_id?.substring?.(3)]},
            (pCommentsData: any) => {
        ...
        queryClient.setQueriesData(
            {
                queryKey: ["thread", data?.link_id?.substring?.(3)]
  ...
  const editCommentMutation = useMutation(
      {mutationFn: ({ parent, text }: { parent: string; text: string }) =>
        ...
        onSuccess: (data) => {
          ...
          queryClient.setQueriesData(
              {queryKey: ["thread", data.permalink.split("/")?.[4]]},
              (pCommentsData: any) =>
  ...
  const commentCollapse = useMutation(
    {
      mutationFn: ({ name, thread, collapse }: any) => dummy(),
      onMutate: async (update) => {
        queryClient.setQueriesData(
            {queryKey: ["thread", update.thread]},
          (pCommentsData: any) =>
  ...
  const commentDelete = useMutation(
    {
      mutationFn: ({ name, thread }: { name: string; thread: string }) => deleteLink({id: name, isPremium: premium?.isPremium}),
      onMutate: async (update) => {
        queryClient.setQueriesData(
            {
                queryKey: ["thread", update.thread]
  ...
  const loadCommentsMutation = useMutation(
    ...
    {
      mutationFn: ({ parentName, children, link_id, permalink, childcomments, token }: any) =>
          loadChildComments(
              parentName,
              children,
              link_id,
